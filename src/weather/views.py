...
def forecast(request):
    ...
    if request.method == "POST":
        ...
        for day in region_forecast["daily"]:
            if (
                unix_to_date(day["dt"]) >= request.POST["start_date"]
                and unix_to_date(day["dt"]) <= request.POST["end_date"]
            ):
...
def archive(request):
    def average(qery):
        avr_day_temp = avr_ngt_temp = avr_humidity = 0
        for day in qery:
    ...
    if request.method == "POST":
        ...
        year = WeatherArchive.objects.filter(
            date__year__gte=date.year, region__icontains=region
        )
        data_year = average(year)

        month = WeatherArchive.objects.filter(
            date__month__gte=date.month, region__icontains=region
        )
        data_month = average(month)

        start_week = date - dt.timedelta(date.weekday())
        end_week = start_week + dt.timedelta(5)
        weeek = WeatherArchive.objects.filter(
            date__range=[start_week, end_week], region__icontains=region
        )
